...
const HallInfoModal: React.FC<Props & ModalRef> = props => {
	...
	useEffect(() => {
		if (props.dev) {
			...
			serviceHall.getHallInfo({ tempId: props.id }).then(rslt => {
				if (rslt.code === 200) {
					setInfoobj(rslt.data)
				}
			})
			...
			<div className="globalScroll">
				{infoobj && (
					<Descriptions>
						...
						<Descriptions.Item label="所属分类" span={3} labelStyle={{ width: "100px" }}>
							<Space>
								{infoobj.styleList &&
									infoobj.styleList.map((item: { styleName: string }) => {
										return item.styleName
									})}
							</Space>
						</Descriptions.Item>
						<Descriptions.Item label="所属行业" span={3} labelStyle={{ width: "100px" }}>
							<Space>
								{infoobj.typeList &&
									infoobj.typeList.map((item: { typeName: string }) => {
