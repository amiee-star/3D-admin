...
const columnFields: Record<string, ColumnType<any>> = {
	...
	templateId: {
		...
		// 					<>
		// 						<img
		// 							src={item.thumbnail ? `${urlFunc.replaceUrl(item.thumbnail, "templateObsUrl")}` : ""}
		// 							width="200"
		// 						/>
		// 					</>
		// 				}
	...
	types: {
		...
		render: (v, item) => {
			...
			item.typeList
				...
				: null
			return types.map(type1 => <p key={type1}>{type1}</p>)
		}
	},
	styles: {
		title: "行业",
		...
		render: (v, item) => {
			const styles: any[] = []
			item.styleList
				? item.styleList.forEach((item1: any) => {
						styles.push(item1.styleName)
				  })
				: null
			return styles.map(style1 => <p key={style1}>{style1}</p>)
		}
	},
	...
	isCheck: {
		...
		ellipsis: true,
		render: v => {
			return <>{v == 1 ? "正常访问" : "禁止访问"}</>
		}
	},
	aliStaticDic: {
		title: "发布状态",
		dataIndex: "aliStaticDic",
		key: "aliStaticDic",
		// fixed: "left",
		width: 150,
		align: "center",
...
export function return4DColumnFields(keys: string[]) {
