...
import fromTableModal from "@/components/modal/fromTable.modal"
...
const OperateForm = () => {
	...
	const deleteHandle = (id: number) => () => {
		Modal.confirm({
			...
			closable: true,
			onOk: () => {
				serviceOperate.deleteForm({ id: id }).then(res => {
					if (res.code === 200) {
	...
	return (
		<Row className="data-form full commentList">
			<Col className="form-search" span={24}>
				<FormSearch fields={returnSearchFiels(["WType", "WStatus", "keyword"])} toSearch={setParams} />
			</Col>
			<Col className="form-result" span={24}>
				<ListTable
					...
					apiService={serviceOperate.formList}
				/>
			</Col>
		</Row>
	)
}
OperateForm.title = "表单管理"
